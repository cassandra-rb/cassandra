...
class CassandraMockTest < CassandraTest
  ...
  def setup
    ...
    @blogs = TwitterCassandra::Mock.new('Multiblog', @test_schema)
    @blogs.clear_keyspace!

    @blogs_long = TwitterCassandra::Mock.new('MultiblogLong', @test_schema)
    @blogs_long.clear_keyspace!

    @type_conversions = TwitterCassandra::Mock.new('TypeConversions', @test_schema)
  ...
  def test_get_range_reversed_slice
    ...
    hash = TwitterCassandra::OrderedHash[data]
    sliced_hash = TwitterCassandra::OrderedHash[data.reverse[1..-1]]
