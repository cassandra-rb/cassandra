...
module SimpleThrift
  ...
  class Field
    ...
    def pack(value)
      [ type.to_i, fid, SimpleThrift.pack_value(type, value) ].pack("cna*")
    end
  end
  ...
  class ThriftService
    ...
    def self.thrift_method(name, rtype, *args)
      arg_struct = SimpleThrift.make_struct("Args__#{name}", *args)
      rv_struct = SimpleThrift.make_struct("Retval__#{name}", SimpleThrift::Field.new(:rv, rtype, 0))
    ...
    def _proxy(method_name, *args)
      ...
      @sock.write(SimpleThrift.pack_request(method_name, arg_struct))
    ...
    [ :i32, :i64, :string ].each { |sym| class_eval "def self.#{sym}; SimpleThrift::#{sym.to_s.upcase}; end" }
  end
end
